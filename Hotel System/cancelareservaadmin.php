...
<?php
$sql = "SELECT reserva.cpf, reserva.id_reserva, reserva.data_inicio, reserva.data_fim, reserva.estado, quartos.tipo, reserva.preco, hoteis.nome, hoteis.cidade
		FROM reserva, quartos, hoteis
		WHERE reserva.id = quartos.id AND quartos.cnpj = hoteis.cnpj AND reserva.estado='reservado'
		ORDER BY reserva.cpf";
...
?>
<form method="post" action="cancelandoreservaadmin.php">
	Digite o ID da Reserva que deseja cancelar (Referente a primeira coluna da tabela): <input type="text" name="cancela"/>
	<input type="submit" value="Efetuar Cancelamento"/>
</form>
<br>
